...
import com.example.administrator.bluetoothseekingforproject.DetailDistance;
...
public class RandomTextView extends FrameLayout implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    //灰色0xdd696969
    private int shadowColor = 0x696969;
...
//            }
            DetailDistance detailDistance=new DetailDistance();
            rssi=detailDistance.getRssi();
            listX.add(Math.abs(rssi/10) * xItem);
            listY.add(Math.abs(rssi/10) * yItem + (yItem >> 2));
